...
// SeiChain is a local docker testnet for a Cosmos SDK chain.
// Implements the ibc.Chain interface.
type SeiChain struct {
	testName      string
	cfg           ibc.ChainConfig
	numValidators int
	numFullNodes  int
...
func NewSeiChain(testName string, chainConfig ibc.ChainConfig, numValidators int, numFullNodes int, log *zap.Logger) *SeiChain {
...
func (c *SeiChain) Nodes() ChainNodes {
	return append(c.Validators, c.FullNodes...)
}

// AddFullNodes adds new fullnodes to the network, peering with the existing nodes.
func (c *SeiChain) AddFullNodes(ctx context.Context, configFileOverrides map[string]any, inc int) error {
...
func (c *SeiChain) Config() ibc.ChainConfig {
	return c.cfg
}

// Implements Chain interface
func (c *SeiChain) Initialize(ctx context.Context, testName string, cli *client.Client, networkID string) error {
	return c.initializeChainNodes(ctx, testName, cli, networkID)
}

func (c *SeiChain) getFullNode() *ChainNode {
	c.findTxMu.Lock()
...
func (c *SeiChain) Exec(ctx context.Context, cmd []string, env []string) (stdout, stderr []byte, err error) {
	return c.getFullNode().Exec(ctx, cmd, env)
}

// Implements Chain interface
func (c *SeiChain) GetRPCAddress() string {
	return fmt.Sprintf("http://%s:26657", c.getFullNode().HostName())
}

// Implements Chain interface
func (c *SeiChain) GetGRPCAddress() string {
...
func (c *SeiChain) GetHostRPCAddress() string {
	return "http://" + c.getFullNode().hostRPCPort
}

// GetHostGRPCAddress returns the address of the gRPC server accessible by the host.
// This will not return a valid address until the chain has been started.
func (c *SeiChain) GetHostGRPCAddress() string {
	return c.getFullNode().hostGRPCPort
}

// HomeDir implements ibc.Chain.
func (c *SeiChain) HomeDir() string {
	return c.getFullNode().HomeDir()
}

// Implements Chain interface
func (c *SeiChain) CreateKey(ctx context.Context, keyName string) error {
	return c.getFullNode().CreateKey(ctx, keyName)
}

// Implements Chain interface
func (c *SeiChain) RecoverKey(ctx context.Context, keyName, mnemonic string) error {
	return c.getFullNode().RecoverKey(ctx, keyName, mnemonic)
}

// Implements Chain interface
func (c *SeiChain) GetAddress(ctx context.Context, keyName string) ([]byte, error) {
...
func (c *SeiChain) BuildWallet(ctx context.Context, keyName string, mnemonic string) (ibc.Wallet, error) {
	if mnemonic != "" {
	...
	return cosmoschain.NewWallet(keyName, addrBytes, mnemonic, c.cfg), nil
}

// BuildRelayerWallet will return a Cosmos wallet populated with the mnemonic so that the wallet can
// be restored in the relayer node using the mnemonic. After it is built, that address is included in
// genesis with some funds.
func (c *SeiChain) BuildRelayerWallet(ctx context.Context, keyName string) (ibc.Wallet, error) {
...
// Implements Chain interface
func (c *SeiChain) SendFunds(ctx context.Context, keyName string, amount ibc.WalletAmount) error {
	return c.getFullNode().SendFunds(ctx, keyName, amount)
}

// Implements Chain interface
func (c *SeiChain) SendIBCTransfer(
	ctx context.Context,
	channelID string,
	keyName string,
	...
	fmt.Println("Sei Chain Send IBC Transfer")
	...
	fmt.Println("Sei Chain get TX", txHash)
	txResp, err := c.getTransaction(ctx, txHash)
	if err != nil {
		return tx, fmt.Errorf("failed to get transaction %s: %w", txHash, err)
	}
	fmt.Println("DONE got TX", txHash)
	if txResp.Code != 0 {
	...
	fmt.Println("DONE - Sei Chain Send IBC Transfer")
	return tx, nil
}

// QueryProposal returns the state and details of a governance proposal.
func (c *SeiChain) QueryProposal(ctx context.Context, proposalID string) (*cosmoschain.ProposalResponse, error) {
...
func (c *SeiChain) PushNewWasmClientProposal(ctx context.Context, keyName string, fileName string, prop cosmoschain.TxProposalv1) (cosmoschain.TxProposal, string, error) {
	tx := cosmoschain.TxProposal{}
	...
	tx, err = c.txProposal(ctx, txHash)
	return tx, codeHash, err
}

// UpgradeProposal submits a software-upgrade governance proposal to the chain.
func (c *SeiChain) UpgradeProposal(ctx context.Context, keyName string, prop cosmoschain.SoftwareUpgradeProposal) (tx cosmoschain.TxProposal, _ error) {
	txHash, err := c.getFullNode().UpgradeProposal(ctx, keyName, prop)
	if err != nil {
		return tx, fmt.Errorf("failed to submit upgrade proposal: %w", err)
	}
	return c.txProposal(ctx, txHash)
}

// SubmitProposal submits a gov v1 proposal to the chain.
func (c *SeiChain) SubmitProposal(ctx context.Context, keyName string, prop cosmoschain.TxProposalv1) (tx cosmoschain.TxProposal, _ error) {
	txHash, err := c.getFullNode().SubmitProposal(ctx, keyName, prop)
	if err != nil {
...
func (c *SeiChain) BuildProposal(messages []cosmosproto.Message, title, summary, metadata, depositStr string) (cosmoschain.TxProposalv1, error) {
	var propType cosmoschain.TxProposalv1
	...
	propType = cosmoschain.TxProposalv1{
		Messages: rawMsgs,
		Metadata: metadata,
		Deposit:  depositStr,
		Title:    title,
...
func (c *SeiChain) TextProposal(ctx context.Context, keyName string, prop cosmoschain.TextProposal) (tx cosmoschain.TxProposal, _ error) {
...
func (c *SeiChain) ParamChangeProposal(ctx context.Context, keyName string, prop *paramsutils.ParamChangeProposalJSON) (tx cosmoschain.TxProposal, _ error) {
...
func (c *SeiChain) QueryParam(ctx context.Context, subspace, key string) (*cosmoschain.ParamChange, error) {
	return c.getFullNode().QueryParam(ctx, subspace, key)
}

func (c *SeiChain) txProposal(ctx context.Context, txHash string) (tx cosmoschain.TxProposal, _ error) {
	txResp, err := c.getTransaction(ctx, txHash)
	if err != nil {
		return tx, fmt.Errorf("failed to get transaction %s: %w", txHash, err)
	}
...
func (c *SeiChain) StoreContract(ctx context.Context, keyName string, fileName string) (string, error) {
	return c.getFullNode().StoreContract(ctx, keyName, fileName)
}

// InstantiateContract takes a code id for a smart contract and initialization message and returns the instantiated contract address.
func (c *SeiChain) InstantiateContract(ctx context.Context, keyName string, codeID string, initMessage string, needsNoAdminFlag bool, extraExecTxArgs ...string) (string, error) {
	return c.getFullNode().InstantiateContract(ctx, keyName, codeID, initMessage, needsNoAdminFlag, extraExecTxArgs...)
}

// ExecuteContract executes a contract transaction with a message using it's address.
func (c *SeiChain) ExecuteContract(ctx context.Context, keyName string, contractAddress string, message string) (txHash string, err error) {
	return c.getFullNode().ExecuteContract(ctx, keyName, contractAddress, message)
}

// QueryContract performs a smart query, taking in a query struct and returning a error with the response struct populated.
func (c *SeiChain) QueryContract(ctx context.Context, contractAddress string, query any, response any) error {
...
func (c *SeiChain) DumpContractState(ctx context.Context, contractAddress string, height int64) (*cosmoschain.DumpContractStateResponse, error) {
...
func (c *SeiChain) StoreClientContract(ctx context.Context, keyName string, fileName string) (string, error) {
...
func (c *SeiChain) QueryClientContractCode(ctx context.Context, codeHash string, response any) error {
	return c.getFullNode().QueryClientContractCode(ctx, codeHash, response)
}

// ExportState exports the chain state at specific height.
// Implements Chain interface
func (c *SeiChain) ExportState(ctx context.Context, height int64) (string, error) {
	return c.getFullNode().ExportState(ctx, height)
}

// GetBalance fetches the current balance for a specific account address and denom.
// Implements Chain interface
func (c *SeiChain) GetBalance(ctx context.Context, address string, denom string) (int64, error) {
...
func (c *SeiChain) AllBalances(ctx context.Context, address string) (types.Coins, error) {
...
func (c *SeiChain) getTransaction(ctx context.Context, txhash string) (*types.TxResponse, error) {
	fn := c.getFullNode()

	return fn.getTransaction(ctx, txhash)
}

func (c *SeiChain) GetGasFeesInNativeDenom(gasPaid int64) int64 {
	gasPrice, _ := strconv.ParseFloat(strings.Replace(c.cfg.GasPrices, c.cfg.Denom, "", 1), 64)
	fees := float64(gasPaid) * gasPrice
	return int64(fees)
}

func (c *SeiChain) UpgradeVersion(ctx context.Context, cli *client.Client, containerRepo, version string) {
...
func (c *SeiChain) pullImages(ctx context.Context, cli *client.Client) {
...
func (c *SeiChain) initializeChainNodes(
	ctx context.Context,
	testName string,
	cli *client.Client,
	networkID string,
...
func (c *SeiChain) Start(testName string, ctx context.Context, additionalGenesisWallets ...ibc.WalletAmount) error {
	chainCfg := c.Config()
...
func (c *SeiChain) Height(ctx context.Context) (uint64, error) {
	return c.getFullNode().Height(ctx)
}

// Acknowledgements implements ibc.Chain, returning all acknowledgments in block at height
func (c *SeiChain) Acknowledgements(ctx context.Context, height uint64) ([]ibc.PacketAcknowledgement, error) {
	var acks []*chanTypes.MsgAcknowledgement
	err := rangeBlockMessages(ctx, c.cfg.EncodingConfig.InterfaceRegistry, c.getFullNode(), height, func(msg types.Msg) bool {
...
func (c *SeiChain) Timeouts(ctx context.Context, height uint64) ([]ibc.PacketTimeout, error) {
...
func (c *SeiChain) FindTxs(ctx context.Context, height uint64) ([]blockdb.Tx, error) {
	fn := c.getFullNode()
	c.findTxMu.Lock()
	defer c.findTxMu.Unlock()
...
func (c *SeiChain) StopAllNodes(ctx context.Context) error {
	var eg errgroup.Group
...
func (c *SeiChain) StartAllNodes(ctx context.Context) error {
...
func (c *SeiChain) VoteOnProposalAllValidators(ctx context.Context, proposalID string, vote string) error {
	var eg errgroup.Group
	for _, n := range c.Nodes() {
		if n.Validator {
			n := n
