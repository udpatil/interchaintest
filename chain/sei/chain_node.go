...
import (
	...
	// TODO: how to override this dependency with the sei chain one for the package specifically
...
func NewChainNode(log *zap.Logger, validator bool, chain *SeiChain, dockerClient *dockerclient.Client, networkID string, testName string, image ibc.DockerImage, index int) *ChainNode {
	tn := &ChainNode{
		log: log,
...
func (tn *ChainNode) NewClient(addr string) error {
	...
	// TODO: does this work?
	tmHttpClient, err := libclient.DefaultHTTPClient(addr)
	if err != nil {
		return err
	}

	tmHttpClient.Timeout = 10 * time.Second

	tmRPC, err := tmrpchttp.NewWithClient(addr, tmHttpClient)
	if err != nil {
		return err
	}
	tn.TMClient = tmRPC
...
func (tn *ChainNode) SetTestConfig(ctx context.Context) error {
	...
	p2p["addr-book-strict"] = false // TODO: doesnt exist??
	...
	consensus["unsafe-propose-timeout-override"] = blockT
	consensus["unsafe-propose-timeout-delta-override"] = blockT
	consensus["unsafe-vote-timeout-override"] = blockT
	consensus["unsafe-vote-timeout-delta-override"] = blockT
	consensus["unsafe-commit-timeout-override"] = blockT
...
func (tn *ChainNode) getTransaction(ctx context.Context, txHash string) (*types.TxResponse, error) {
	// Retry because sometimes the tx is not committed to state yet.
	txResp := types.TxResponse{}
	err := retry.Do(func() error {
		var err error
		fmt.Println("Getting TX", txHash)
		stdout, _, err := tn.ExecQuery(ctx, "tx", txHash)
		if err != nil {
			fmt.Println("err", err)
			return err
		}
		cdc := tn.Chain.Config().EncodingConfig.Codec
		err = cdc.UnmarshalJSON([]byte(stdout), &txResp)
		if err != nil {
			fmt.Println("cdc err", err)
			return err
		}
		fmt.Println("err", err)
		return err
...
func (tn *ChainNode) InstantiateContract(ctx context.Context, keyName string, codeID string, initMessage string, needsNoAdminFlag bool, extraExecTxArgs ...string) (string, error) {
	...
	txResp, err := tn.getTransaction(ctx, txHash)
	if err != nil {
...
func (tn *ChainNode) QueryProposal(ctx context.Context, proposalID string) (*cosmoschain.ProposalResponse, error) {
	stdout, _, err := tn.ExecQuery(ctx, "gov", "proposal", proposalID)
	if err != nil {
		return nil, err
	}
	var proposal cosmoschain.ProposalResponse
...
func (tn *ChainNode) SubmitProposal(ctx context.Context, keyName string, prop cosmoschain.TxProposalv1) (string, error) {
...
func (tn *ChainNode) UpgradeProposal(ctx context.Context, keyName string, prop cosmoschain.SoftwareUpgradeProposal) (string, error) {
	command := []string{
		"gov", "submit-proposal",
...
func (tn *ChainNode) TextProposal(ctx context.Context, keyName string, prop cosmoschain.TextProposal) (string, error) {
	command := []string{
		"gov", "submit-proposal",
		"--type", "text",
		"--title", prop.Title,
...
func (tn *ChainNode) QueryParam(ctx context.Context, subspace, key string) (*cosmoschain.ParamChange, error) {
	stdout, _, err := tn.ExecQuery(ctx, "params", "subspace", subspace, key)
	if err != nil {
		return nil, err
	}
	var param cosmoschain.ParamChange
...
func (tn *ChainNode) DumpContractState(ctx context.Context, contractAddress string, height int64) (*cosmoschain.DumpContractStateResponse, error) {
	...
	res := new(cosmoschain.DumpContractStateResponse)
	if err := json.Unmarshal([]byte(stdout), res); err != nil {
		return nil, err
	}
	return res, nil
